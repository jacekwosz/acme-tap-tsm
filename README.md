...
# Work in progress - README for ACME Fitness on TAP with TSM

To setup your Spring Cloud Gateway instance and HTTP Proxy, copy `values-sample.yaml` to `values.yaml`, edit the values, and run:

```
ytt -f scg/gateway.yaml -f scg/scg-httpproxy.yaml -f scg/authserver.yaml -f scg/authserver-httpproxy.yaml -f scg/clientreg.yaml -f values.yaml --ignore-unknown-comments | kubectl apply -f-
```
